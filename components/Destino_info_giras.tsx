...
import { ChevronRight, TrainFrontTunnelIcon } from "lucide-react";
  ...
  return (
    <Link href={`/detalle-programa/${IdPrograma}`} className="font-semibold">
      <div className="w-full bg-whites rounded-2xl shadow-2xl">
        ...
        <div className="flex flex-col relative  p-6 bg-white rounded-b-2xl">
          <h4 className="text-sm">
            {/* <div className="flex pb-3">
            <MapPin className="" /> <span className="pl-2">{Titulo}</span>
          </div> */}
            <div className="bg-yellow-400 font-bold text-black w-[50%] left-[180px] text-center p-2 absolute -mt-[50px] rounded-md">
              {subtitulo ? subtitulo : "Destacado"}
            </div>
          </h4>
          <h3 className="text-xl font-bold text-[#58167D]">
            <div className="flex text-[24px] ">
              {TrainFrontTunnelIcon === undefined ? (
                <span>(Confirmar con Agente)</span>
              ) : (
                <span className="">
                  <MapPin className="inline mb-1" />
                  {Titulo ? Titulo : "Sin información"}
                  <br></br>
          ...
          <small className="text-[.rem] mb-2 font-bold">
